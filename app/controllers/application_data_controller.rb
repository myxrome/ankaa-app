class ApplicationDataController < ActionController::Base
  ...
  def index
    ...
    result = ActiveRecord::Base.connection.execute lighting_query(params[:k], params[:u])
    render json: result.first['d'].to_s
  end

  private
  def lighting_query(keys, date)
"set work_mem = '16MB';
with tg as (
...
p as (
    ...
    '/content/p/' || p.\"value_id\" || '/' || p.\"id\" || '/' || p.\"file_key\" || 'xhdpi' || '.jpg' as l,
    p.\"updated_at\" as u
    from promos p, _v v
    where p.\"value_id\" = v.\"id\"
),
v as (
    select v,
    json_agg(row_to_json((d))) as d,
    json_agg(row_to_json((p))) as p
    from _v v, d d, p p
    where d.\"_id\" = v.\"id\"
    and p.\"_id\" = v.\"id\"
    group by v
